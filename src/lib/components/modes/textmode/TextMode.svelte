...
import { type NodeIterator, type SyntaxNode, type SyntaxNodeRef, type Tree } from '@lezer/common'
...
function createCustomFoldService() {
  return foldService.of((state, start, end) => {
    const tree = ensureSyntaxTree(state, state.doc.length, Number.POSITIVE_INFINITY)
...
function foldRecursive(path: JSONPath = [], recursive = true) {
  const state = codeMirrorView.state
  const docLength = state.doc.length
  const tree = ensureSyntaxTree(state, docLength, Number.POSITIVE_INFINITY)
  if (!tree) {
    return
  }
 ...
 */
export function openTransformModal({ id, rootPath, onTransform, onClose }: TransformModalOptions) {
  try {
    const json = parser.parse(text)
