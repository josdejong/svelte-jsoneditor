...
export function patch(
  ...
  sortedColumn = callback?.sortedColumn !== undefined ? callback.sortedColumn : patchedSortedColumn
  text = undefined
  textIsRepaired = false
...
export function findElement(path: JSONPath): Element | undefined {
  ...
  return refContents?.querySelector(`td[data-path="${encodeDataPath(resolvedPath)}"]`) ?? undefined
}
